...
class clientSocketHanlder(socketHandler):   
    ...
    def send_strings_data(self,data_str:str,max = 1):
        """
        ...
        for line in data_str:
            ...
            if count_str_grouped >= max:                        # Trigger send
                self._socket.sendall(buffer_str.encode())      # Its necesary convert to binay ( str.encode, by default utf-8)
                response += self._socket.recv(100).decode()     # Its necesary convert to str ( binary.decode, by default utf-8)
